...
void ConstraintTable::insert2CAT(const Path& path)
{
    if (cat.empty())
    {
     ...
     // assert(cat_goals[path.back().location] == MAX_TIMESTEP); // We comment this out because it is possible that two non-goal locations end at the same location in a PBS node
